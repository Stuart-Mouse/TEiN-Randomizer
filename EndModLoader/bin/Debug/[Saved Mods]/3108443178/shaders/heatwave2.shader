...
varying vec2 worldPos;
    ...
    void frag(){
        ...
        vec2 rippleval = vec2(sin(timer*2+worldPos.y / 20.0) * .005, sin(timer*3+worldPos.y / 10.0) * .002) * ratio;
        
        vec4 outcolora = texture(framebuf, screencoords.xy);
        vec4 outcolorb = texture(framebuf, screencoords.xy + rippleval);
        vec4 outcolor = mix(outcolora, outcolorb, .25);
